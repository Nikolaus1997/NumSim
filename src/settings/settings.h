...
struct Settings
{
  std::array<int,2> nCells{200,20};          //< number of cells in x and y direction
  std::array<double,2> physicalSize{200.,20.}; //< physical size of the domain
  double re = 147.135;                 //< reynolds number
  double endTime = 150.0;             //< end time of the simulation
  double tau = .6;                  //< safety factor for time step width
  double maximumDt = 1.0;            //< maximum time step width
  double L_lbm = 100.;
